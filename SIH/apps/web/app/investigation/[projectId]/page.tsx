...
export default function ProjectInvestigationPage({ params }: PageProps) {
  ...
  const initialProject =
    MOCK_PROJECTS.find(
      (p) =>
        p.header.projectId.toLowerCase() === urlProjectId.toLowerCase() ||
        urlProjectId.includes(p.header.projectId)
    ) || MOCK_PROJECTS[0];

  const [activeProject, setActiveProject] = useState<ProjectInvestigationData>(initialProject);
